...
async function callClaudeWithCache(anthropic, model, systemPrompt, userMessage, maxTokens) {
  ...
  if (!combinedText) {
    throw new Error('AI response was empty or missing text content')
  }
...
/**
 * Build user message prompt for generating A+ setups
 * @param {object} params - Setup generation constraints
 * @returns {string} Formatted prompt for Claude
 */
function buildGenerateSetupsPrompt({
  market_bias,
  timeframe,
  risk_profile,
  focus_sectors,
  ...
  return `A+ setup generation request:
- Market Bias: ${market_bias || 'neutral'}
- Preferred Timeframe: ${timeframe || 'swing'}
- Risk Profile: ${risk_profile || 'balanced'}
- Focus Sectors: ${Array.isArray(focus_sectors) && focus_sectors.length > 0 ? focus_sectors.join(', ') : 'none'}
- Watchlist: ${Array.isArray(watchlist) && watchlist.length > 0 ? watchlist.join(', ') : 'none provided'}
- Max Setups: ${Number(max_setups) || 5}
- Notes: ${notes || 'none'}
- Price Context: ${price_context || 'unavailable'}

Generate only high-conviction opportunities and return exactly this JSON structure:
{
  "market_context": "<2-3 concise sentences>",
  "risk_notes": ["<risk note>", "<risk note>"],
  "setups": [
    {
      "symbol": "<ticker>",
      ...
      "catalyst": "<short catalyst>",
      "invalidation": "<single invalidation rule>"
    }
  ]
}`
}
...
/**
 * Parse and validate setup generator response from Claude
 * @param {string} raw - Raw JSON from Claude
 * @returns {object} Validated setup response
 */
function parseGenerateSetupsResponse(raw) {
  const d = safeParseJSON(raw)
  const validGrades = new Set(['A+', 'A', 'A-'])
  const validDirections = new Set(['long', 'short'])
  const validConfidence = new Set(['low', 'moderate', 'high'])
  const validTimeframes = new Set(['intraday', 'swing', 'position'])

  const setups = Array.isArray(d.setups)
    ? d.setups.map((s) => {
      const grade = String(s?.setup_grade ?? 'A').toUpperCase()
      const direction = String(s?.direction ?? 'long').toLowerCase()
      const confidence = String(s?.confidence ?? 'moderate').toLowerCase()
      const timeframe = String(s?.timeframe ?? 'swing').toLowerCase()

      return {
        symbol: String(s?.symbol ?? '').toUpperCase(),
        direction: validDirections.has(direction) ? direction : 'long',
        setup_grade: validGrades.has(grade) ? grade : 'A',
        thesis: String(s?.thesis ?? ''),
        entry: Number(s?.entry ?? 0),
        stop: Number(s?.stop ?? 0),
        target_1: Number(s?.target_1 ?? 0),
        target_2: s?.target_2 == null ? null : Number(s.target_2),
        risk_reward: Number(s?.risk_reward ?? 0),
        confidence: validConfidence.has(confidence) ? confidence : 'moderate',
        timeframe: validTimeframes.has(timeframe) ? timeframe : 'swing',
        catalyst: String(s?.catalyst ?? ''),
        invalidation: String(s?.invalidation ?? ''),
      }
    }).filter((s) => s.symbol && Number.isFinite(s.entry) && Number.isFinite(s.stop) && Number.isFinite(s.target_1))
    : []

  return {
    market_context: String(d.market_context ?? ''),
    risk_notes: Array.isArray(d.risk_notes) ? d.risk_notes.map(String).slice(0, 5) : [],
    setups,
  }
}
