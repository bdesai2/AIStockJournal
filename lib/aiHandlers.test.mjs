...
describe('AI Handlers', () => {
  ...
  describe('parsePotentialTradeResponse', () => {
    it('parses a valid potential trade response', () => {
      const raw = JSON.stringify({
        setup_quality: 'strong',
        rr_potential: 2.5,
        key_levels: { entry: 150, stop: 145, target: 160 },
        bullish_case: 'Strong support holding',
        bearish_case: 'Breaking below support',
        probability: 'high',
        recommendation: 'consider',
      })

      const result = parsePotentialTradeResponse(raw)
      expect(result.setup_quality).toBe('strong')
      expect(result.rr_potential).toBe(2.5)
      expect(result.key_levels.entry).toBe(150)
      expect(result.probability).toBe('high')
      expect(result.recommendation).toBe('consider')
    })

    it('defaults to "moderate" for invalid setup_quality', () => {
      const result = parsePotentialTradeResponse('{"setup_quality": "invalid"}')
      expect(result.setup_quality).toBe('moderate')
    })

    it('defaults to "watch" for invalid recommendation', () => {
      const result = parsePotentialTradeResponse('{"recommendation": "unknown"}')
      expect(result.recommendation).toBe('watch')
    })

    it('defaults to "medium" for invalid probability', () => {
      const result = parsePotentialTradeResponse('{"probability": "uncertain"}')
      expect(result.probability).toBe('medium')
    })

    it('handles null rr_potential', () => {
      const result = parsePotentialTradeResponse('{"rr_potential": null}')
      expect(result.rr_potential).toBeNull()
    })

    it('converts string numbers to actual numbers in key_levels', () => {
      const raw = JSON.stringify({
        key_levels: { entry: '150', stop: '145', target: '160' },
      })

      const result = parsePotentialTradeResponse(raw)
      expect(typeof result.key_levels.entry).toBe('number')
      expect(result.key_levels.entry).toBe(150)
      expect(result.key_levels.stop).toBe(145)
      expect(result.key_levels.target).toBe(160)
    })

    it('provides sensible defaults for missing fields', () => {
      const result = parsePotentialTradeResponse('{}')
      expect(result.setup_quality).toBe('moderate')
      expect(result.rr_potential).toBeNull()
      expect(result.key_levels.entry).toBe(0)
      expect(result.bullish_case).toBe('')
      expect(result.bearish_case).toBe('')
      expect(result.probability).toBe('medium')
      expect(result.recommendation).toBe('watch')
    })
      ...
      const raw = `\`\`\`json
{
  ...
  "rr_potential": 3.0,
  "key_levels": {"entry": 100, "stop": 90, "target": 130},
...
\`\`\``
      ...
      const result = parsePotentialTradeResponse(raw)
      expect(result.setup_quality).toBe('excellent')
      expect(result.rr_potential).toBe(3.0)
