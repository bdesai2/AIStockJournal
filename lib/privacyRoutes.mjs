  ...
  app.post('/api/privacy/delete-account', authenticateRequest, deleteLimiter, async (req, res) => {
    ...
    try {
      ...
      if (strategyIds.length > 0) {
        const { data: strategyShots, error: strategyShotsReadErr } = await adminClient
          .from('strategy_screenshots')
          .select('storage_path')
          .in('strategy_id', strategyIds)
        if (strategyShotsReadErr && !isRelationMissing(strategyShotsReadErr)) throw strategyShotsReadErr

        const strategyPaths = (strategyShots || []).map((s) => s.storage_path).filter(Boolean)
        if (strategyPaths.length > 0) {
          await adminClient.storage.from('trade-screenshots').remove(strategyPaths)
        }
      ...
      const { data: userSetups, error: setupErr } = await adminClient
        .from('setups')
        .select('id')
        .eq('user_id', userId)
      if (setupErr && !isRelationMissing(setupErr)) throw setupErr

      const setupIds = (userSetups || []).map((s) => s.id)
      if (setupIds.length > 0) {
        const { data: setupShots, error: setupShotsReadErr } = await adminClient
          .from('setup_screenshots')
          .select('storage_path')
          .in('setup_id', setupIds)
        if (setupShotsReadErr && !isRelationMissing(setupShotsReadErr)) throw setupShotsReadErr

        const setupPaths = (setupShots || []).map((s) => s.storage_path).filter(Boolean)
        if (setupPaths.length > 0) {
          await adminClient.storage.from('trade-screenshots').remove(setupPaths)
        }

        const { error: setupShotsErr } = await adminClient
          .from('setup_screenshots')
          .delete()
          .in('setup_id', setupIds)
        if (setupShotsErr && !isRelationMissing(setupShotsErr)) throw setupShotsErr
      }

      const { error: setupDeleteErr } = await adminClient.from('setups').delete().eq('user_id', userId)
      if (setupDeleteErr && !isRelationMissing(setupDeleteErr)) throw setupDeleteErr
