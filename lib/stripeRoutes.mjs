...
import { isValidUuid, isValidEmail, safeErrorMessage } from './validators.mjs'
...
// Service-role client for audit log writes (not subject to user RLS)
...
export function registerStripeRoutes(app) {
  ...
  app.post('/api/stripe/start-trial', async (req, res) => {
    ...
    try {
      ...
      await writeAuditLog(user.id, 'TRIAL_STARTED', { trialEndsAt: trialEndsAt.toISOString() })
      ...
      res.status(500).json({
        message: safeErrorMessage(err, 500),
      })
    }
  })
  ...
  app.post('/api/stripe/cancel-subscription', async (req, res) => {
    ...
    try {
      ...
      await writeAuditLog(user.id, 'SUBSCRIPTION_CANCELED', {
      ...
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          if (session.subscription) {
            ...
            await writeAuditLog(userId, 'SUBSCRIPTION_UPGRADED', {
        ...
        case 'customer.subscription.deleted': {
          ...
          await writeAuditLog(userId, 'SUBSCRIPTION_CANCELED', {
  ...
  /**
   * Write an audit log entry using the admin (service-role) Supabase client so
   * it is not subject to per-user RLS restrictions.  Falls back to the anon
   * client if the service-role client is not available.
   */
  async function writeAuditLog(userId, action, details = {}) {
    ...
    if (error) console.error(`[audit] Failed to write ${action} for ${userId}:`, error.message)
  }
  ...
  app.post('/api/admin/grant-pro', requireAdmin, async (req, res) => {
    ...
    try {
      ...
      await writeAuditLog(req.user.id, 'ADMIN_GRANT_PRO', {
        targetUserId: userId,
      ...
      res.status(500).json({ error: safeErrorMessage(err, 500) })
    }
  })
  ...
  app.post('/api/admin/revoke-pro', requireAdmin, async (req, res) => {
    ...
    try {
      ...
      await writeAuditLog(req.user.id, 'ADMIN_REVOKE_PRO', {
        targetUserId: userId,
