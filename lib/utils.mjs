...
export function safeParseJSON(raw) {
  ...
  // Aggressive cleanup: remove common text patterns that wrap JSON
  let cleanedForExtraction = cleaned
    .replace(/^.*?[\{\[]/s, (match) => match.includes('{') || match.includes('[') ? match : '')
    .replace(/[\}\]].*?$/s, (match) => match.includes('}') || match.includes(']') ? match : '')

  const attempts = [
    cleaned,
    cleaned.replace(/\n/g, '\\n'),
    cleanedForExtraction,
    cleanedForExtraction.replace(/\n/g, '\\n'),
  ]
